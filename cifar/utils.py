...
def train(model, optimizer, trdat, tsdat, args, nesterov_factor):
    rec_names = ["iter", "loss", "acc", "backwardnfe", "forwardnfe", "time/iter", "time"]
    rec_unit = ["","","","","","s","min"]
    itrcnt = 0
    ...
    for epoch in range(1, args.niters+1):
        ...
        if epoch % 2 == 0:
            ...
            loss /= dsize
            acc /= dsize
            printouts = [epoch, loss.detach().cpu().numpy(), acc.detach().cpu().numpy(), None, str(model[1].df.nfe / bcnt), test_time, (time.time()-start_time)/60]
