...
parser.add_argument('--names', nargs='+', default='NnodeHighRes', help="List of models to run")
parser.add_argument('--log-file', default='NnodeHighRes', help="name of the logging csv file")
...
for tries in range(1):
    print(args.start)
    print(args.end)
    hblayer = NODElayer(NesterovNODEHighRes(DF(dim, hidden), None), tol=args.tol, evaluation_times=torch.Tensor([args.start, args.end]))
    ...
    train(model, optimizer, trdat, tsdat, args, args.names, csvname=f'imgdat/{evaluation_times_folder}/{args.log_file}_{args.tol}_seed_{randomSeed}.csv', gamma=model[1].df.gamma)
