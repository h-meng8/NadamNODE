...
trdat, tsdat = mnist(batch_size=256, path_to_data='./mnist_data')
...
def model_gen(name, gpu, **kwargs):
    if name == 'node':
        ...
        model = nn.Sequential(anode_initial_velocity(1, dim),
                              layer, predictionlayer(dim)).to(device=gpu)
    elif name == 'anode':
        dim = 6
        nhid = 64
        evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
        ...
        model = nn.Sequential(hbnode_initial_velocity(1, dim, nhid),
                              hblayer, predictionlayer(dim, truncate=True)).to(device=gpu)
    elif name == 'sonode':
        dim = 5
        nhid = 50
        evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
        ...
        model = nn.Sequential(hbnode_initial_velocity(1, dim, nhid),
                              layer, predictionlayer(dim, truncate=True)).to(device=gpu)
    elif name == 'ghbnode':
        dim = 5
        nhid = 50
        evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
        ...
        model = nn.Sequential(hbnode_initial_velocity(1, dim, nhid),
                              layer, predictionlayer(dim, truncate=True)).to(device=gpu)
    elif name == 'nesterovnode2':
        dim = 5
        nhid = 50
        evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
        layer = NODElayer(NesterovNODE2(DF(dim, nhid), None, use_h=True, sign=-1), nesterov_algebraic=True, time_requires_grad=False, evaluation_times=evaluation_times,  **kwargs)
        model = nn.Sequential(hbnode_initial_velocity(1, dim, nhid),
                              layer, predictionlayer(dim, truncate=True)).to(device=gpu)
    elif name == 'gnesterovnode2':
        dim = 5
        nhid = 50
        evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
        layer = NODElayer(NesterovNODE2(DF(dim, nhid), actv_h=hard_tanh_half, actv_df=hard_tanh_half, corr=2.0, corrf=False, use_h=True, sign=-1), nesterov_algebraic=True, activation_h=hard_tanh_half, time_requires_grad=False, evaluation_times=evaluation_times,  **kwargs)
        model = nn.Sequential(hbnode_initial_velocity(1, dim, nhid),
                              layer, predictionlayer(dim, truncate=True)).to(device=gpu)
    elif name == 'nesterovnode3':
        dim = 5
        nhid = 50
        evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
        layer = NODElayer(NesterovNODE3(DF(dim, nhid)), time_requires_grad=False, evaluation_times=evaluation_times,  **kwargs)
        model = nn.Sequential(hbnode_initial_velocity(1, dim, nhid),
                              layer, predictionlayer(dim, truncate=True)).to(device=gpu)
    # elif name == 'nadamnode':
    #     dim = 5
    #     nhid = 50
    #     evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
    #     layer = NODElayer(NadamNODE1(DF(dim, nhid), None, use_h=True, sign=-1), nesterov_algebraic=True, adam=True, time_requires_grad=False, evaluation_times=evaluation_times,  **kwargs)
    #     model = nn.Sequential(adamnode_initial_velocity(1, dim, nhid),
    #                           layer, predictionlayer(dim, truncate=True)).to(device=gpu)
    # elif name == 'nadamnode2':
    #     dim = 5
    #     nhid = 50
    #     evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
    #     layer = NODElayer(NadamNODE2(DF(dim, nhid), None, use_h=True, sign=-1), nesterov_algebraic=True, adam=True, time_requires_grad=False, evaluation_times=evaluation_times,  **kwargs)
    #     model = nn.Sequential(adamnode_initial_velocity(1, dim, nhid),
    #                           layer, predictionlayer(dim, truncate=True)).to(device=gpu)
    elif name == 'nadamnode3':
        dim = 5
        nhid = 50
        evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
        layer = NODElayer(NadamNODE3(DF(dim, nhid), sign=-1), time_requires_grad=False, evaluation_times=evaluation_times,  **kwargs)
        model = nn.Sequential(adamnode_initial_velocity(1, dim, nhid),
                              layer, predictionlayer(dim, truncate=True)).to(device=gpu)

    elif name == 'gnesterovnode_euler':
        dim = 5
        nhid = 50
        evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
        method = "euler"
        step_size = 0.2
        layer = NODElayer(NesterovNODE(DF(dim, nhid), actv_h=hard_tanh_half, actv_df=hard_tanh_half, corr=2.0, corrf=False, use_h=True, sign=-1), nesterov_algebraic=True, activation_h=hard_tanh_half, time_requires_grad=False, evaluation_times=evaluation_times, method=method, step_size=step_size, **kwargs)
        model = nn.Sequential(hbnode_initial_velocity(1, dim, nhid),
                              layer, predictionlayer(dim, truncate=True)).to(device=gpu)
    elif name == 'nadamnode_euler':
        dim = 5
        nhid = 50
        evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
        method = "euler"
        step_size = 0.2
        layer = NODElayer(NadamNODE3(DF(dim, nhid)), time_requires_grad=False, evaluation_times=evaluation_times, method=method, step_size=step_size, **kwargs)
        model = nn.Sequential(adamnode_initial_velocity(1, dim, nhid), layer, predictionlayer(dim, truncate=True))

    # elif name == 'gnadamnode':
    #     dim = 5
    #     nhid = 50
    #     evaluation_times = torch.Tensor([1, 2]).to(device=gpu)
    #     layer = NODElayer(NadamNODE1(DF(dim, nhid), sign=-1, actv_h=hard_tanh_half, actv_df=hard_tanh_half, corr=2.0, corrf=False), nesterov_algebraic=True, adam=True, time_requires_grad=False, evaluation_times=evaluation_times,activation_h=hard_tanh_half, **kwargs)
    #     model = nn.Sequential(adamnode_initial_velocity(1, dim, nhid),
    #                           layer, predictionlayer(dim, truncate=True)).to(device=gpu)
    ...
    for name in args.names:
        ...
        if not args.no_run:
            ...
            train_out = train(model, optimizer, trdat, tsdat, args, name, 0, evalfreq=1,
                        csvname=f'imgdat/{evaluation_times_folder}/{args.log_file}_{args.tol}_lr_{args.lr}_seed_{randomSeed}.csv')
