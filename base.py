...
class NODEintegrate(nn.Module):
    ...
    def __init__(self, df, shape=None, tol=1e-5, adjoint=True, evaluation_times=None, recf=None, nesterov_algebraic=None, activation_h=None, activation_output=None, time_requires_grad=True, verbose=True, adam=False, method="dopri5", step_size=0.2):
        """
    ...
    def forward(self, x0):
        ...
        if self.shape:
            ...
            if self.method != "dopri5":
                out = odeint(self.df, x0, self.evaluation_times, rtol=self.tol, atol=self.tol, method=self.method, options={"step_size":self.step_size})
            else:
            ...
            if self.method != "dopri5":
                ...
                out = odeint(self.df, x0, self.evaluation_times, rtol=self.args.tol, atol=self.tol)
...
NNODE2 = NesterovNODE2 # Alias

class NesterovNODE3(NODE):
    def __init__(self, df, actv_h=None, gamma_guess=-3.0, gamma_act='sigmoid', corr=-100, corrf=True, sign=1, shift=0):
        super().__init__(df)
        # Momentum parameter gamma
        self.gamma = Parameter([gamma_guess], frozen=False)
        self.gammaact = nn.Sigmoid() if gamma_act == 'sigmoid' else gamma_act
...
class NesterovNODEHighRes(NODE):
    ...
    def dFdx(self, t, x):
        ### derivative of F w.r.t states "x"
        Ahat = torch.autograd.functional.jacobian(lambda x: self.df(t, x), inputs=x, vectorize=True)
        # print(Ahat)
        # print(Ahat.squeeze().reshape(batch*d, -1))
        # print(Ahat.squeeze())
        #Ahat = Ahat.squeeze().reshape(batch*d, -1)
        return Ahat
